...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_GET["order"]) && isset($_POST["confirm"])) {
        ...
        if ($result) {
            ...
            header("location: " . url() . 'canceled.php');
            exit();
        }
            ...
            <div class="pt-6 px-4">
                ...
                <?php if (isset($_GET['order']) && $_GET['order']) :
                    ...
                    <div class="w-full bg-white shadow rounded-lg mb-4 p-4 sm:p-6 h-full">
                        ...
                        <a href="<?= url() . 'canceled.php' ?>" class="text-cyan-600 hover:text-cyan-700">
                ...
                <?php else : ?>
                    <div class="bg-white shadow rounded-lg mb-4 p-4 sm:p-6 h-full">
                        <h3 class="text-2xl font-bold leading-none text-gray-900 mb-4">Canceled Orders</h3>
                        <?php
                        $result = $conn->query("SELECT * FROM `orders` WHERE `status`=5 ORDER BY `id` DESC");
                        $orders = $result->fetch_all(MYSQLI_ASSOC);
